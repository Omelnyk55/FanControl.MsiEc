...
```json
{
  ...
  // (1..100, 0 = disable). Default 100: full-speed requests act instantly.
  "coolerBoostAtDuty": 100
}
```

### About fan response speed

The MSI EC ramps the actual PWM toward the requested duty at only ~1 %/s
(measured: 3000→5900 RPM takes ~56 s through the curve-table path) — firmware
behavior, same under MSI Center, not adjustable through documented registers.
The plugin compensates for the case that matters: when FanControl requests
`coolerBoostAtDuty` % or more, it flips MSI's **Cooler Boost** bit, which
bypasses the ramp entirely (3000→6200 RPM in ~2 s, slightly above the normal
100 % speed). Below the threshold (with 5 % hysteresis) boost is released and
the fan follows the normal slow ramp down.
